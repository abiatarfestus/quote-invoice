...
class OrderListTab:
    def __init__(self, notebook, parent_frame, order_details_tab, session):
        ...
        y_scroll.grid(column=1, row=0, sticky=(N, W, E, S))
        x_scroll.grid(column=0, row=1, sticky=(N, W, E, S))
        ...
        self.open_order_btn.grid(column=0, row=1, sticky=(S, N, W, E))
        ...
        self.add_order_btn.grid(column=1, row=1, sticky=(S, N, W, E))
        ...
        self.search_order_btn.grid(column=4, row=1, sticky=(S, N, W, E))
    ...
    def list_orders(self, orders, from_customer=False):
        ...
        if not from_customer:
            ...
            customers = self.session.query(Customer).filter(
                Customer.customer_id == orders[0].customer_id
            )
    ...
    def open_blank_order_form(self):
        ...
        self.notebook.select(
            6
        )  # Change to tab name instead of index (same on customer)
    ...
    def search_by_order_id(self, order_id=""):
        if not order_id:
            error_message = messagebox.showerror(
                message="Cannot search with a blank Order ID.", title="Error"
            )
            return error_message
        try:
