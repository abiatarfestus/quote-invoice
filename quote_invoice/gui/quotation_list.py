...
class QuotationListTab:
    def __init__(self, notebook, parent_frame, quote_details_tab, session):
        ...
        self.quotations = (
            self.session.query(Quotation).order_by(Quotation.quote_date).all()
        )
    ...
    def list_quotations(self, quotations, from_customer=False):
        ...
        if not from_customer:
            ...
            customers = self.session.query(Customer).filter(
                Customer.customer_id == quotations[0].customer_id
            )
    ...
    def view_quotation(self):
        ...
        quotation = db.get_quotations(
            self.session, pk=self.selected_quotation["values"][0]
        )
    ...
    def search_by_quote_id(self, quote_id=""):
        if not quote_id:
            error_message = messagebox.showerror(
                message="Cannot search with a blank Quote ID.", title="Error"
            )
            return error_message
        try:
