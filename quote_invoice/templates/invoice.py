...
class Invoice:
    def __init__(self, session, order_id, templates_dir="", output_dir=""):
        ...
        if self.settings and self.settings.invoice_template:
            ...
            self.invoice_output_file = os.path.join(
                self.invoice_output_folder, "generated_invoice.docx"
            )
        else:
    ...
    def generate_invoice_preview(self):
        ...
        try:
            ...
            raise Exception(
                f"{e} Check if you have another open invoice and close it or save it with a different name. Then try again."
            )
